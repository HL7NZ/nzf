...
A Request heading is displayed as follows:

[ps_request.sa_form] Special Authority for [ps_request.for_entitlements]
<Case When Attribute> only from <Case Applicant element content> (or <Case Applicant element content>).  Approvals valid for <Case Term element content> <Case Term Measure Attribute> for applications meeting the following criteria:
The request criteria are stored in a math block.  There are a number of variations on the way this is unpicked.  In simple terms each <apply> tag represents a level of indentation in the criteria, e.g. 1, 1.1, 1.1.1.  As such these can be nested.  Typically an <apply> will begin with <and/> or <or/> to indicate the logical relationship of the list items contained in the <apply>.  <and/> usually means that the block of text begins with either ‘Both’ or ‘All of the following’ and each list item except the last ends with ‘and’.  <or/> usually means that the block of text begins with either ‘Either’ or ‘Any of the following’ and each list item except the last ends with ‘or’.
